...
_RotRect::_RotRect(Sprite s)
{
	S.x = s.sprite.getLocalBounds().width * s.sprite.getScale().x;
	S.y = s.sprite.getLocalBounds().height * s.sprite.getScale().y;
	C.x = s.sprite.getPosition().x - s.sprite.getOrigin().x + S.x / 2;
	C.y = s.sprite.getPosition().y - s.sprite.getOrigin().y + S.y / 2;
	ang = s.getAngle();
}

bool _RotRect::check_two_rotated_rects_intersect(_RotRect * rr1, _RotRect * rr2)
{
...
void _RotRect::AddVectors2D(_Vector2D * v1, _Vector2D * v2)
{
	v1->x += v2->x; v1->y += v2->y;
}
void _RotRect::SubVectors2D(_Vector2D * v1, _Vector2D * v2)
{
	v1->x -= v2->x; v1->y -= v2->y;
}

void _RotRect::RotateVector2DClockwise(_Vector2D * v, float ang)
{
